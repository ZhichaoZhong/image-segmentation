...
def display_result(image, soft_image, seg, gt,  figsize=(12, 12)):
    ...
    ax[1].imshow(soft_image, cmap="gray")
    ax[1].set_axis_off()
    ax[1].set_title("Soft-masked image", fontsize=12)
    fig.show()

    ax[2].imshow(seg, cmap="gray")
    ax[2].set_axis_off()
    ax[2].set_title("Segmentaion", fontsize=12)

    ax[3].imshow(gt, cmap="gray")
    ax[3].set_axis_off()
    ax[3].set_title("Ground truth", fontsize=12)
    fig.show()
...
soft_image = boosted_seg._apply_soft_mask(image_orig)

display_result(image_orig, soft_image, mask_bs, image_mask)
